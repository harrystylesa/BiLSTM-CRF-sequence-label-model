...
class NERTrainer(BaseTrain):
    def __init__(self, sess, model, train_data, val_data, config):
        ...
        embedding = self.loadWord2Vec(self.config['word2vecfile'])
        self.sess.run(self.model.embedding_init, feed_dict={self.model.embedding_placeholder: embedding})

    def loadWord2Vec(self, filename):
        df = pd.read_csv(filename, encoding='utf-8')
        embedding = df.iloc[:, 1:].apply(pd.to_numeric).values
        return embedding
